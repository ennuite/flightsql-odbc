...
FlightSqlConnection::BuildFlightClientOptions(const ConnPropertyMap &properties,
  ...
  if (send_ping_frame) {
    auto ping_interval = GetPingFrameInterval(properties);
    auto ping_timeout = GetPingFrameTimeout(properties);
    auto max_pings_without_data = GetMaxPingsWithoutData(properties);
